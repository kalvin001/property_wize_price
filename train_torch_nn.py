...
def train_torch_nn_model(data_file, output_dir='model', test_size=0.2, random_state=42, 
                         hidden_layers=(256, 128, 64), dropout_rate=0.2, learning_rate=0.001,
                         batch_size=64, num_epochs=200, patience=20):
    """
    训练PyTorch深度神经网络模型
    ...
    torch_model = ModelFactory.create_model('torch_nn', **model_params)
    ...
    # 保存模型
    model_path = os.path.join(output_dir, "torch_nn_model.pt")
    ...
    # 保存特征重要性
    importance_path = os.path.join(output_dir, "torch_nn_feature_importance.csv")
    ...
    # 保存详细的评估指标
    metrics_path = os.path.join(output_dir, "torch_nn_metrics.json")
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    
    # 前端展示目录
    frontend_metrics_path = os.path.join("frontend", "public", "data", "torch_nn_metrics.json")
...
def main():
    parser = argparse.ArgumentParser(description='训练PyTorch深度神经网络模型')
